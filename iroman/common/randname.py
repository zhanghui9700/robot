...
def nickname():
    lns, fns = LAST_NAMES, FIRST_NAMES
    _last = _last_name(1, lns)
    _first = _first_name(random.randint(2, 2), _last, fns)
    return "%s%s" % (_last, _first)
...
def email():
    return "%s%s@qq.com" %  (random.randint(1, 10),
                            _random_name(10, string.digits))
